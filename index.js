  ...
  for (let i = 0; i < games.length; i++) {
    ...
    div.innerHTML = `
      ...
      games[i].name
                        ...
                        <p>Money raised: <strong>$${games[
                          i
                        ].pledged.toLocaleString("en-US")}</strong></p>
                        <p>Money Goal: <strong>$${games[i].goal.toLocaleString(
                          "en-US"
                        )}</strong></p>
                        `;
...
contributionsCard.innerHTML = `<h5>${totalContributions.toLocaleString(
  "en-US"
)}</h5>`;
...
const statement = `<p>A total of $${totalMoney.toLocaleString(
  "en-US"
)} has been raised for ${
  GAMES_JSON.length
} games. Currently, ${totalUnfunded} ${
  totalUnfunded <= 1 ? "1 game remains unfunded." : "games remain unfunded."
